...
locals {
  ...
  cname_failover_records     = map([for record in var.records_cname : record if lookup(record, "failover_type", null) != null])
  cname_records              = map([for record in var.records_cname : record if lookup(record, "failover_type", null) == null])
}
