...
locals {
  ...
  cname_failover_records     = { for key, val in var.records_cname : key => val if contains({ key : val }, "failover_type") }
  cname_records              = { for key, val in var.records_cname : key => val if ! contains({ key : val }, "failover_type") }
}
