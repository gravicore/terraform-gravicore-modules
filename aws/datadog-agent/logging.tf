...
module "logging_kms_key" {
  ...
  description = join(" ", [var.desc_prefix, "KMS key for Datadog"])
...
resource "aws_iam_role" "logging" {
  ...
  assume_role_policy = data.aws_iam_policy_document.logging_lambda[0].json
}
...
resource "aws_iam_role_policy" "logging_log_groups" {
  ...
  role   = aws_iam_role.logging_log_groups[0].id
  policy = data.aws_iam_policy_document.logging_log_groups[0].json
}

resource "aws_cloudwatch_log_group" "logging" {
  count = var.create ? length(var.cloudwatch_log_groups) : 0
...
resource "aws_lambda_function" "logging" {
  ...
  tags          = local.tags
  filename = substr(
    data.archive_file.logging_lambda[0].output_path,
    2,
    -1
  )

  # filename         = data.archive_file.logging_lambda[0].output_path
  source_code_hash = data.archive_file.logging_lambda[0].output_base64sha256
  kms_key_arn      = module.logging_kms_key.key_arn

  role    = aws_iam_role.logging[0].arn
...
resource "aws_lambda_permission" "allow_cloudwatch" {
  ...
  function_name = aws_lambda_function.logging[0].arn
