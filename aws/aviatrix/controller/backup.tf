...
module "s3_backup" {
  source    = "git::https://github.com/cloudposse/terraform-aws-s3-bucket.git?ref=v4.10.0"
  enabled   = var.create
  namespace = ""
  stage     = ""
...
module "parameters_backup" {
  source      = "git::https://github.com/gravicore/terraform-gravicore-modules.git//aws/parameters?ref=0.52.0"
  providers   = { aws = "aws" }
