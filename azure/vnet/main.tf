resource "azurerm_virtual_network" "default" {
  ...
  name                = join(var.delimiter, [local.stage_prefix, "vnet", local.region])
...
resource "azurerm_subnet" "public" {
  ...
  name                = join(var.delimiter, [local.stage_prefix, "snet", "public", local.region, index(local.vnet_public_subnets, each.key) + 1])
  resource_group_name = var.resource_group_name
...
resource "azurerm_subnet" "private" {
  ...
  name                = join(var.delimiter, [local.stage_prefix, "snet", "private", local.region, index(local.vnet_private_subnets, each.key) + 1])
  resource_group_name = var.resource_group_name
...
resource "azurerm_subnet" "internal" {
  ...
  name                = join(var.delimiter, [local.stage_prefix, "snet", "intra", local.region, index(local.vnet_internal_subnets, each.key) + 1])
  resource_group_name = var.resource_group_name
