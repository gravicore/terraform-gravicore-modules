...
resource "azurerm_subnet" "public" {
  ...
  name                 = join(var.delimiter, [local.module_prefix, "public", index(local.vnet_public_subnets, each.key) + 1])
  resource_group_name  = var.resource_group_name
...
resource "azurerm_subnet" "private" {
  ...
  name                 = join(var.delimiter, [local.module_prefix, "private", index(local.vnet_private_subnets, each.key) + 1])
  resource_group_name  = var.resource_group_name
  virtual_network_name = concat(azurerm_virtual_network.default.*.name, [""])[0]
  address_prefixes     = [each.key]
}
